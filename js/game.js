...
const WordsyGame = (() => {
    ...
    const STATE_KEY = 'wordsy-game-state';
    const MAX_HINTS = 3;
    ...
    async function loadPuzzle(number) {
        ...
        try {
            ...
            console.log(`[Wordsy] Puzzle #${number} loaded — ${Object.keys(puzzleData).length} words`);
            return true;
        } catch (e) {
    ...
    function saveState() {
        try {
            ...
            console.warn('[Wordsy] Failed to save state:', e);
        }
    }
    ...
    function shareResult() {
        ...
        const text = WordsyStats.generateShareText(puzzleNumber, guesses.length, guesses);
    ...
    function handleWin() {
        WordsyStats.recordWin(puzzleNumber, guesses.length);
        WordsyUI.disableInput();

        setTimeout(() => {
            WordsyUI.showWinOverlay(targetWord, guesses.length);
        }, 700);
    }
    ...
    async function init() {
        ...
        // Initialize UI
        WordsyUI.init();
        WordsyUI.setPuzzleNumber(puzzleNumber);

        // Load puzzle data
        const loaded = await loadPuzzle(puzzleNumber);
        if (!loaded) {
        ...
        // Restore any saved state for this puzzle
        ...
        console.log(`[Wordsy] Game initialized — Puzzle #${puzzleNumber}, target: [hidden]`);
    }
